...
var images = {
    ...
    Background2 : "./assets/sprites/2bg.png",
...
function loadmainmenu(){
        ...
        y2.onclick = () => {       
            ...
            help.enabled = true; 
            setTimeout( ()=>{
                window.resources.get(images.BG).currentTime=0;
                window.resources.get(images.BG).loop = true;
                if(window.resources.get(images.BG).paused)
                {
                    window.resources.get(images.BG).play();
                }
...
function loadgameUI(){
    ...
    y.onclick = () => {
        clearUI();
        loadpauseUI();
        window.resources.get(images.BG).pause();
...
var main = () =>{
    ...
    {     
        ...
        case gameStates.menu:  
            ...
            cxt.fillText("TOTAL SCORE",cav.width/2,(cav.height * 0.1) + 550/2 );
            cxt.font = "70px GameFont";            
            cxt.fillText(totalScore,cav.width/2,(cav.height * 0.1) + 550/2 + 70);
            cxt.fillText("HIGH SCORE",cav.width/2,(cav.height * 0.1) + 500/2 + 140 + 70 );                      
            cxt.fillText(highScore,cav.width/2,(cav.height * 0.1) + 500/2 + 140 + 140);
            cxt.textAlign = "start"; 
            },2);
        ...
        case gameStates.orientation:
            ...
            cxt.drawImage(window.resources.get(images.P_L),0,0,window.resources.get(images.P_L).width*2,window.resources.get(images.P_L).height/2);
            },11);
            break;
        default:         
        break;       
...
{     
    ...
    let b1 = window.resources.get(images.Background2);
    ...
    window.drawCall(
        ()=>{ 
            cxt.drawImage(b1,0,0,b1.width,b1.height);            
    },0.5); 
...
function drawBar (){
            ...
            cxt.lineWidth = 5;
            cxt.strokeText(game_properties.TapPower,232,95+ offset); 
            cxt.fillText(game_properties.TapPower,232,95+ offset); 

            cxt.fillStyle = "#ffcc00";
            cxt.fillRect(245,46+offset,267 * (game_properties.cp/game_properties.cmax),46);

            cxt.strokeStyle = "#000000";
            cxt.lineWidth = 4;
            cxt.strokeRect(245,46+offset,267,46);
             
}

function tapInc (value=1){
    game_properties.cp +=value;
    if(game_properties.cp>=game_properties.cmax)
    {
        game_properties.TapPower++;
        game_properties.cp=0;
        game_properties.cmax += 9;
        playSound(images.ding);
        entitylist.push(new Level(game_properties.TapPower));
    }
}
...
(function(){
    ...
    function submitScore(){
        ...
        tr.open("POST","http://gamesandapp.com/lbtest/usertotalscore.php",true);
        tr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        tr.setRequestHeader('X-PINGOTHER', 'pingpong');
        tr.send("score="+Score);
        ...
        let tr2 = new XMLHttpRequest();        
        tr2.open("POST","http://gamesandapp.com/lbtest/userleaderboard.php",true);
        tr2.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        tr2.setRequestHeader('X-PINGOTHER', 'pingpong');
        tr2.send("score="+Score);
        tr2.onreadystatechange = ()=>{
            if(tr2.readyState==4 && tr2.status=="200")
            {
                console.log("changed1");
                console.log(tr2.responseText);
            }
        }
    }
