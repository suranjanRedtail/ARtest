...
var images = {    
    ...
    waves :"./assets/sprites/waves.png",  
...
function resetgame() {
    ...
    entitylist =[];
    player_entity.pos.x = cav.width;
    player_entity.pos.y = -player_entity.size.y * 0.8;
    player_entity.state = "waiting";
...
{    
    ...
    fishsprites = [
        new Sprite(images.fish),
        new Sprite(images.fish2),
        new Sprite(images.frog),
    ]

    for (let index = 0; index < 8; index++) {
        let t = new Sprite( images.waves,720 * index,0,720,305);
        wave_sprites.push(t);        
    }
    wave_sprites.reverse();
...
var main = () =>{
    ...
    {     
        ...
        case gameStates.game:
            ...
            timer += dt;
            game_properties.game_time += dt; 
            game_properties.time_left -= dt;
            if(game_properties.time_left <= 0)
            {
                gameOver();
            }
            ...
            if(timer>nexttimer)
            {
                entitylist.push( new Pickup(Math.floor( Math.random()*2), 200 *( Math.random()>0.5?1:-1) ));
                timer = 0;
            }
            ...
            drawCall(()=>{
                ...
                cxt.font = "bold 100px GameFont";   
                cxt.lineWidth = 8;         
                cxt.strokeText(Math.floor(game_properties.time_left),cav.width/2,95);
                cxt.fillStyle = score_color;
                cxt.font = "bold 100px GameFont";
                cxt.fillText(Math.floor(game_properties.time_left),cav.width/2,95);  
                cxt.textAlign = "start";  
...
class Player extends Entity{   
    ...
    {
        super(images.hook);
        this.pos.y = -this.size.y * 0.8;
        this.pos.x = -this.size.x/2 + cav.width/2;
        this.tpos = this.pos;
        this.state = "waiting";
        ...
        this.update = ()=> 
        {
            ...
            {
                let delta = this.tpos.y - this.pos.y;
                if(delta > 1)
                {
                    this.pos.y += game_properties.hook_speed * dt * Math.sign(delta);
                }
                else
                {
                    this.pos = Object.assign({},this.tpos); 
                    this.state = "returning";                   
                }
            }
            if(this.state == "returning")
            {
                let delta = -this.size.y* 0.8 - this.pos.y;
                if(delta <= 0)
                {
                    this.pos.y += game_properties.hook_speed * dt * Math.sign(delta);
                }
        ...
        this.hook = (mPos)=>{
            ...
            {
                mPos.x -= this.size.x/2;
                this.tpos = mPos;
                this.pos.x = this.tpos.x;
                this.tpos.y -= this.size.y - 66;
                this.state = "hooking";
            }
        } 
    }
...
class Pickup extends Entity{
    ...
    {
        super( type == 1?fishsprites[2]: speed>0?fishsprites[0]:fishsprites[1]);
        this.type = type;
        let random_roll = Math.random();
        let size_roll = 0.5 + random_roll*0.5;
        this.size.x *= size_roll;
        this.size.y *= size_roll;
        this.speed = speed * (0.5 + random_roll*0.5);
        let index = 3 +Math.floor(random_roll * 5);
        this.order = -2 + (index * 0.125);
        this.basey = 280 + 55*(index * (1 + 0.2 * index));
        this.score = Math.floor( 10 * (1+ random_roll));
        this.hooked = false;

        this.pos.x = speed>0? -this.size.x: mainviewport.width; 
        ...
        this.update = ()=>{
            if(this.hooked)
            {
                this.pos.x = player_entity.center.x; 
                this.pos.y = player_entity.center.y; 
                this.pos.x -= this.size.x/2;
                this.pos.y -= this.size.y/2;

                if( player_entity.state == "waiting")
                {
                this.enabled = false;
                }
            }
            ...
            this.pos.y = this.basey + Math.sin( (game_properties.game_time*(-game_properties.wave_speed )) + (index * Math.PI/3.5))* (10 +( -5 + 2*index ));
            }

            if(this.pos.x < -this.size.x || this.pos.x > mainviewport.width)
            {
                this.enabled=false;
            }
            ...
            if( !this.hooked && dist < distmax)
            {
                ...
                else
                {
                    ...
                    game_properties.Score =  Math.max(0,game_properties.Score);
...
{   
    ...
    for (let index = 0; index < wave_sprites.length; index++) {
        const element = wave_sprites[index];
        let yloc = 270 + 45*(index * (1 + 0.2 * index))+ Math.sin( (game_properties.game_time*(-game_properties.wave_speed )) - (index * Math.PI/3.5))* (10 +( -5 + 2*index ));
        drawCall(
            ()=>{
            element.draw(cxt,0,yloc,element.width,element.height)           
        },-2 + (index * 0.125));
    }
