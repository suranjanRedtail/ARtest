...
var images = {    
    ...
    player :"./assets/sprites/player_225.png",   
    coin : "./assets/sound/coin.wav",      
    BG : "./assets/sound/BG.mp3", 
    tap_1 : "./assets/sound/bounce.wav", 
    hives : "./assets/sprites/hives.png",
    bees : "./assets/sprites/bee.png",
};
...
var text_color = "#e8a600";
var outline_color = "#e8a600";
...
function resetgame() {
    ...
    player_entity.currentlane = 0;
    player_entity.state = "climbing";
...
var main = () =>{
    ...
    {        
        ...
        case gameStates.game:
            mainviewport.update();           
            mainviewport.posy -= dt * game_properties.player_speed;
            timer += dt;

            game_properties.game_time += dt;
            game_properties.player_speed += 3 * dt;

            if(timer>nexttimer)
            {
                entitylist.push(new Pickup());
                timer =0;
                nexttimer = 1 + Math.random()*5;
            }
            ...
            if(last_bee ==  null)
            {
                ...
                entitylist.push(tb);
                last_bee = tb;
                nextbeedist = 300 + Math.random() * 500;
            }
            else if( last_bee.pos.y + last_bee.size.y > mainviewport.posy)
            {
                let tb = new Enemy();
                entitylist.push(tb);
                tb.pos.y = last_bee.pos.y - nextbeedist;
                last_bee = tb;                
                nextbeedist = 300 + Math.random() * 500;
            }
...
class Player extends Entity{   
    constructor()
    {
        let psprite = new SpriteAnimation( images.player,0,0,[2,3],225,236,4,1);
        let pjumpSprites = new SpriteAnimation( images.player,0,0,[0,1],225,236,0,0);
        pjumpSprites.play = false;
        super(psprite);
        this.oposy = 912;
        this.pos.y = 912;
        this.currentlane = 0;
        ...
        this.update = ()=>{
            this.oposy -= game_properties.player_speed *dt;
            
            let delta = planes[this.targetlane] - this.pos.x;
            let percent = Math.abs(delta) / (planes[1]-planes[0]);

            this.pos.y = this.oposy - ( Math.sin( Math.PI * percent) * game_properties.jump_height);
            ...
            {                                   
                ...
                {
                    this.state = "jumping";
                    delta = dt * 720 * Math.sign(delta);
                    this.pos.x +=delta;
                } 
                if((this.targetlane == 0 && delta>=0) || (this.targetlane == 1 && delta<=0) )
                {
                    this.pos.x = planes[this.targetlane];
...
class Pickup extends Entity{
    constructor()
    {
        let pickupsprite = new Sprite(images.hives,0,0,300,263);
        super(pickupsprite);

        let rand_roll = (Math.random()+1)*0.5;;
        this.size.x *= rand_roll;
        this.size.y *= rand_roll;

        this.pos.x = cav.width/2 -this.size.x/2;
        this.pos.y = mainviewport.posy - this.size.y;
        

        this.update = () => {
            if(this.pos.y > mainviewport.posy + mainviewport.height)
            {
                this.enabled = false;
            }
            ...
            if( dist < distmax)
                {
                    game_properties.Score+= Math.floor((1+rand_roll)*10);
                    this.enabled = false; 
...
class Enemy extends Entity{
    constructor()
    {
        let sprites = [new Sprite(images.bees,0,0,498/2,308) , new Sprite(images.bees,498/2,0,498/2,308)];

        super(sprites[Math.floor( Math.random()*1.99)]);
        this.size.x/=2;
        this.size.y/=2;

        this.pos.x = planes[ Math.floor( Math.random()*1.99)] + player_entity.size.x/2 - this.size.x/1.5 ;
        this.pos.y = mainviewport.posy - this.size.y;
...
{   
    let p1 = resources.get(images.Background);
    let p1x = -Math.floor(mainviewport.posy % p1.height);
    drawCall(
        ()=>{
        cxt.drawImage(p1,0,p1x,p1.width,p1.height);
        cxt.drawImage(p1,0,p1x - p1.height + 1,p1.width,p1.height);   
    },-2);    
}
