...
var images = {
    ...
    P_SafaB : "./assets/sprites/Tempo/P_tempoB.png",
    P_SafaW : "./assets/sprites/Tempo/P_tempoW.png",
    P_TukTukR : "./assets/sprites/Tempo/P_tuktukR.png",
    P_TukTukY : "./assets/sprites/Tempo/P_tuktukY.png",  
    P_truck : "./assets/sprites/Tempo/truck_front.png",
    ...
    treenew : "./assets/sprites/Props/side_tree.png",
    ...
    coin_sound : "./assets/sprites/coin.wav",
};
...
var game_properties ={
    ...
    background_scroll_acceleration :{ x:0,y:0.02},
    enable_shadows:true,
    pause:false,
    playerCar:null,
    Score:0,
...
{    
    ...
    player= new Player(images.P_truck,0,0,20);
    player.init();
...
var main = () =>{
    ...
    switch(currentGameState)
    {
        ...
        case gameStates.game:{
            ...
            game_properties.ScoreReal += dt*2;
...
class Enemy extends Entity{
    constructor(sprite,sizeX,sizeY,speed)
    {
        ...
        this.update = () => {
            ...
            if(player.enabled){
            if(physics.checkCollision(this,player.collider))
            {    
                if(false)
                {
                    game_properties.ScoreReal += 20;
                    this.enabled=false;
                    this.create_new();
                }
                else
                {
                    ...
                    player.enabled = false;                
                    explosion.pos = Object.assign( {},player.pos);
        ...
        this.create_new = ()=>{
            this.enabled = false; 
            let index = Math.floor(enemyCarSpriteList.length * Math.random());         
            let tempE = new Enemy(enemyCarSpriteList[index],0,0,game_properties.enemy_current_speed);
...
class Player extends Entity{
    constructor(sprite,sizeX,sizeY,speed)
    {
        ...
        this.tc = new Entity(null,100,190);
        this.order = 0.02;
        this.update = () => {
            if(this.enabled){
    ...
    get collider (){
        this.tc.pos.x = this.pos.x + 5;
        this.tc.pos.y = this.pos.y + 1;
        return this.tc;
    }  
}
...
class Coin extends Entity{
    constructor(){
        super(images.coin,60,60);
        this.pos.y = -this.size.y;
        this.pos.x = lanes[ Math.floor(Math.random()*2.99)]- this.size.x/2;
        this.time =0;
        this.speed =5;
        this.order = 0.01;
        this.update =()=>{
            this.time +=dt;
            this.pos.y += Math.sin(this.time * this.speed)*0.75;
            this.pos.y += game_properties.background_scroll_speed.y;
            if(physics.checkCollision(this,player.collider) && currentGameState==gameStates.game)
            {
                game_properties.ScoreReal+=10;
                resources.get(images.coin_sound).currentTime=0;
                resources.get(images.coin_sound).play();
                this.enabled=false;
            }
            if(this.pos.y >cav.height)
            {
...
function prepare_obstacles() {
    ...
    t = new obstaclesdetails(images.treenew,0,0);
    t.offset = cav.width - resources.get(images.cone).width;
    t.posx = 10;
    t.order = 2;
    obstacles.push(t);
}
