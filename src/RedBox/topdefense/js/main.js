...
var images = {    
    ...
    enemy_animation : "./assets/sprites/animations/140_170.png",
...
{    
    ...
    player_entity = new Player(player_sa,250,250);
    ...
    {        
        ...
        case gameStates.game:
            ...
            if(counter> next_interval)
            {
                ...
                let temp = new Balls( 0,2,colors[ Math.floor( Math.random() * colors.length)]);
                temp.pos.x = Math.random()* cav.width * 0.6 + cav.width* 0.2;
                temp.pos.y = - temp.size.y;
                entitylist.push(temp);
                counter =0;
                next_interval =  game_properties.spawn_interval +  Math.random() ;
            }
...
class Balls extends Entity{
    constructor(radius,count,speed,color)
    {
        ...
        super(new SpriteAnimation(images.enemy_animation,0,0,[4,3,2,1,0],140,170,12,1),0,0);
        ...
        this.update = () => {
            ...
            if(this.done)
            {
                ...
                if(this.radius < 1)
                {
                    ...
                    temp_anim.pos.x = this.pos.x; 
                    temp_anim.pos.y = this.pos.y; 
        ...
        this.checkmouse=(x,y,damage)=>{
            let distfrommouse = Math.sqrt( Math.pow( this.center.x - x,2) + Math.pow( this.center.y - y,2));
            
            if(distfrommouse < this.radius_getter)
            {
...
class Player extends Entity{
    constructor(sprite,sizex,sizey){
        ...
        this.shoot =() => {            
            ...
            {                
                this.has_shot = true;
                let tarrow = new Arrow( 1500, this.pos.x + this.size.x/2 , this.pos.y - (118/2),1);
